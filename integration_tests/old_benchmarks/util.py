...
def get_job(benchmark: str | None, instance_path: str | None) -> JobInstanceRich:
    ...
    if benchmark is not None and instance_path is not None:
        ...
        if benchmark.startswith("j1"):
            import integration_tests.old_benchmarks.job1 as job1  # ty:ignore[unresolved-import]
            ...
            import integration_tests.old_benchmarks.generators as generators  # ty:ignore[unresolved-import]

            instance = generators.get_job()
        elif benchmark.startswith("matmul"):
            import integration_tests.old_benchmarks.matmul as matmul  # ty:ignore[unresolved-import]

            instance = matmul.get_job()
        elif benchmark.startswith("dist"):
            import integration_tests.old_benchmarks.dist as dist  # ty:ignore[unresolved-import]

            instance = dist.get_job()
        elif benchmark.startswith("dask"):
            import integration_tests.old_benchmarks.dask as dask  # ty:ignore[unresolved-import]
