...
python -m cascade.benchmarks dist --job $JOB --idx $SLURM_PROCID --controller-url $CONTROLLER_URL --hosts $EXECUTOR_HOSTS --workers-per-host $WORKERS_PER_HOST --shm-vol-gb $SHM_VOL_GB 2>&1 | tee $LOGDIR
# *** ------- ***
