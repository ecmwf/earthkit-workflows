...
def logParse(files: Iterable[str]) -> dict[str, pd.DataFrame]:
    ...
    microtraces_struct = [
        {"kind": k.value, "duration_ns_sum": v.duration_ns_sum, "duration_ns_cnt": v.duration_ns_cnt, "duration_ns_avg": v.duration_ns_sum / v.duration_ns_cnt if v.duration_ns_cnt > 0 else math.nan}
