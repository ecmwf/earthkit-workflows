...
from concurrent.futures import Future, ThreadPoolExecutor, Executor as PythonExecutor, wait, FIRST_COMPLETED, ALL_COMPLETED
from time import time_ns

from cascade.executor.runner.memory import ds2shmid
from cascade.executor.comms import Listener, callback, send_data
from cascade.executor.msg import BackboneAddress, DatasetTransmitCommand, DatasetTransmitPayloadHeader, DatasetTransmitPayload, DatasetTransmitFailure, DatasetPublished, Syn, Ack, DatasetPurge
            ...
            try:
                ...
                for m in self.dlistener.recv_messages(resend_grace_ms):
                    ...
                    if isinstance(m, DatasetTransmitCommand):
                        ...
                        if m.ds in self.invalid:
                            raise ValueError(f"unexpected transmit command {m} as the dataset was already purged")
                ...
                for e in queue:
                    ...
                    if command in self.futs_in_progress:
                        raise ValueError(f"asked for retry of {command}, but said future still in progress")
