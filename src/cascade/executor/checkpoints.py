    ...
    match spec.storage_type:
        case "fs":
            if not isinstance(spec.storage_params, str):
                ...
                raise CascadeUserError(f"expected checkpoint storage params to be str, gotten {spec.storage_params.__class__}")
...
def build_persist_command(checkpoint_spec: CheckpointSpec | None, ds: DatasetId, hostId: HostId) -> DatasetPersistCommand:
    if checkpoint_spec is None:
        ...
        raise CascadeUserError(f"unexpected persist need when checkpoint storage not configured")
    id_ = checkpoint_spec.persist_id
    if not id_:
        from cascade.low.exceptions import CascadeUserError

        raise CascadeUserError(f"unexpected persist need when there is no persist id")
    ...
    match spec.storage_type:
        case "fs":
            if not spec.persist_id:
                ...
                raise CascadeUserError("unexpected list persisted when there is no persist id")
...
def build_retrieve_command(checkpoint_spec: CheckpointSpec | None, ds: DatasetId, hostId: HostId) -> DatasetRetrieveCommand:
    if checkpoint_spec is None:
        ...
        raise CascadeUserError(f"unexpected retrieve need when checkpoint storage not configured")
    id_ = checkpoint_spec.retrieve_id
    if not id_:
        from cascade.low.exceptions import CascadeUserError

        raise CascadeUserError(f"unexpected retrieve when there is no retrive id")
...
def possible_repersist(dataset: DatasetId, checkpointSpec: CheckpointSpec | None) -> None:
    ...
    if not checkpointSpec:
        ...
        raise CascadeUserError(f"unexpected repersist when checkpoint storage not configured")
    if not checkpointSpec.retrieve_id:
        from cascade.low.exceptions import CascadeUserError

        raise CascadeUserError(f"unexpected repersist when no retrieve id")
    if not checkpointSpec.persist_id:
        from cascade.low.exceptions import CascadeUserError

        raise CascadeUserError(f"unexpected repersist when no persist id")
